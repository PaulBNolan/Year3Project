...
Menu::Menu(Game& t_game,sf::Font t_font, sf::RenderWindow &t_window, KeyHandler const & t_keyHandler, XboxController& t_controller) :
	m_game(t_game), m_font(t_font), m_keyhandler(t_keyHandler), m_controller(t_controller)
{
	...
	m_optionsText[0].setString("Start Game");
	m_optionsText[1].setString("Tutorial");
	m_optionsText[2].setString("Options");
	m_optionsText[3].setString("Exit");
...
void Menu::update(double t)
{
	handleKeyInput(t);
}

void Menu::handleKeyInput(double t)
{
	...
	if ((m_keyhandler.isPressed(sf::Keyboard::Enter) && m_enterReleased) || m_controller.m_currentState.A && !m_controller.m_previousState.A)
	{
		...
		case 1:
			...
			break;
		case 2:
			m_game.setGameStates(GameStates::Option); // change the game state to option
			break;
		case 3:
			m_game.setGameStates(GameStates::Quit); // quit the app
			break;
		default:
			break;
		}
